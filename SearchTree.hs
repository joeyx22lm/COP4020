...
successor a (Node b l r)
    | (b > a) && (successor a l) == Nothing = Just b
	| (b > a) = (successor a l)
	| otherwise = (successor a r)

foldT g a Nil = a
foldT g a (Node b left right) = (b `g` (foldT g a right)) `g` (foldT g a left)
...
closest :: Int -> Tree Int -> Int
closest a (Node b left right) = foldT (\x y -> (closer a x y)) b (Node b left right)
