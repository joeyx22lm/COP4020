...
diff (Set (x:xs)) (Set ys)
    | subSet (Set [x]) (Set ys) = diff (Set xs) (Set ys)
    | otherwise = union (Set [x]) (diff (Set xs) (Set ys))
diff empty a = empty

symDiff :: (Ord a) => Set a -> Set a -> Set a
symDiff (Set a) (Set b) = union (diff (Set a) (Set b)) (diff (Set b) (Set a))

powerSet :: (Ord a) => Set a -> Set (Set a)
powerSet (Set xs)
    | xs == [] = Set [empty]
    | otherwise = Set (map (makeSet) (powerSet' xs))
